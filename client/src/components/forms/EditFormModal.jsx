...
const EditFormModal = ({ isOpen, onClose, onFormUpdated, formData }) => {
  ...
  useEffect(() => {
    if (formData && isOpen) {
      ...
      setStartTime(
        formData.startTime
          ? new Date(formData.startTime).toLocaleString("en-In", {
      ...
      setEndTime(
        formData.endTime
          ? new Date(formData.endTime).toLocaleString("en-In", {
              timeZone: "Asia/Kolkata",
            })
          : ""
      );
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      onFormUpdated(res.data.data);
      onClose();
    } catch (err) {
      console.error("Form update error", err?.response?.data || err);
      const errorMessage =
      ...
      >
        ...
        <div className="flex flex-wrap gap-3 justify-center overflow-y-scroll lg:overflow-y-hidden">
          {backgrounds.map((bg, i) => (
            <div
              key={i}
              className={`relative w-24 md:w-28 h-16 md:h-20 lg:w-[47%] lg:h-[14%] rounded-lg overflow-hidden border-2 hover:scale-105 transition-all duration-300 ${
